...
# 导入各个模块的关键功能
# 从 sample.py 导入
from gflownet.priors.sample import (
...
# 从 data_gen.py 导入
from gflownet.priors.data_gen import (
...
from gflownet.priors.tree_evaluation import (
    predict_with_tree,
...
from gflownet.priors.json_tree import process_json_trees, create_similarity_calculator
from gflownet.priors.gfn_trees import compare_trees

from gflownet.priors.graph_viz import plot_tree

# --- New top-level helper for inner parallelization ---
def _compare_trees_worker(args_tuple):
    """
    Worker function for ProcessPoolExecutor to compare a single input tree with a single prior tree.
    """
    input_tree, prior_tree, feature_names, classes_, bounds, comp_dist, dist_weight = args_tuple
    # compare_trees is already imported in this module
...
def sample_and_generate_trees(
    ...
    if output_dir is None:
        ...
        base_output_dir = f"gflownet/priors/data/{timestamp}"
    else:
...
def create_similarity_calculator(json_file_path, save_maps=True, output_dir='.', num_inner_workers: int = None):
    """
    创建一个函数，用于计算输入树与JSON文件中所有树的平均相似度 (并行版本)
    
    Args:
        json_file_path (str): JSON文件路径，包含要比较的树
        save_maps (bool): 是否保存特征和类别映射到文件
        output_dir (str): 保存映射的目录
        num_inner_workers (int, optional): 内部并行比较时使用的进程数。
                                          None 表示使用 os.cpu_count()。
        
    Returns:
        function: 一个函数，接受一个树作为输入，返回其与JSON文件中所有树的平均相似度
    """
    # 预处理JSON文件中的树
    # print(f"[{os.getpid()}] create_similarity_calculator: Processing JSON trees from {json_file_path}")
    ...
    if not valid_trees:
        print(f"Warning: No valid trees found in JSON file: {json_file_path}. Similarity will be 0.")
        # Return a lambda that always gives 0 similarity if no valid prior trees
        return lambda input_tree, comp_dist=True, dist_weight=0.5, bounds=None: 0.0
        
    # print(f"[{os.getpid()}] create_similarity_calculator: Loaded {len(valid_trees)} valid trees for similarity calculation from {json_file_path}.")
    
    # 创建比较函数 (并行版本)
    def calculate_average_similarity_parallel_inner(input_tree, comp_dist=True, dist_weight=0.5, bounds=None):
        """
        计算输入树与JSON文件中所有树的平均相似度 (此函数由 create_similarity_calculator 返回)
        ...
        if not valid_trees: # Should have been caught above, but as a safeguard
            # This print might be redundant if the one in create_similarity_calculator already fired
            # print("Warning: No valid trees to compare with (parallel_inner).")
            return 0.0
            
        # Prepare bounds if None
        actual_bounds = bounds
        if actual_bounds is None and feature_names:
            actual_bounds = [(0, 1) for _ in range(len(feature_names))]

        tasks = []
        for prior_tree_from_list in valid_trees:
            tasks.append((
                input_tree,
                prior_tree_from_list,
                feature_names,
                classes_,
                actual_bounds, 
                comp_dist,
        ...
        # Determine number of workers for this inner loop
        # If num_inner_workers is 0 or 1, run sequentially.
        num_workers_to_use = num_inner_workers
        if num_workers_to_use is None:
            num_workers_to_use = os.cpu_count() # Default to all CPUs if not specified
        
        # Ensure at least 1 worker, and not more workers than tasks.
        # Also, only use ProcessPoolExecutor if num_workers_to_use > 1 and there's more than 1 task.
        num_workers_to_use = max(1, min(num_workers_to_use, len(tasks)))

        if num_workers_to_use > 1 and len(tasks) > 1:
            from concurrent.futures import ProcessPoolExecutor # Import here for worker context if needed
            # print(f"[{os.getpid()}] Inner parallel for input tree: {len(tasks)} prior trees, {num_workers_to_use} workers.")
            with ProcessPoolExecutor(max_workers=num_workers_to_use) as executor:
                similarities = list(executor.map(_compare_trees_worker, tasks))
        else:
            # print(f"[{os.getpid()}] Inner sequential for input tree: {len(tasks)} prior trees.")
            similarities = [_compare_trees_worker(task_args) for task_args in tasks]
    ...
    return calculate_average_similarity_parallel_inner
...
def calculate_average_similarity(
    ...
    # print(f"[{os.getpid()}] calculate_average_similarity: Creating similarity calculator for {priors_json} with num_inner_workers={num_inner_workers}")
    try:
        # Pass num_inner_workers to the new create_similarity_calculator
        similarity_calculator = create_similarity_calculator(priors_json, save_maps=False, num_inner_workers=num_inner_workers)
    except FileNotFoundError:
        print(f"Error: Prior_json file {priors_json} not found in calculate_average_similarity. Returning 0.0 similarity.")
        return 0.0
    except Exception as e:
        print(f"Error creating similarity calculator in calculate_average_similarity: {e}. Returning 0.0 similarity.")
        return 0.0

    # 直接使用预处理的 new_tree_numerical 调用计算器
    # print(f"[{os.getpid()}] calculate_average_similarity: Calling calculator for new_tree.")
    try:
        avg_sim = similarity_calculator(
            input_tree=new_tree_numerical,
            comp_dist=comp_dist,
            dist_weight=dist_weight, 
            bounds=bounds
        )
        # print(f"[{os.getpid()}] calculate_average_similarity: Calculated avg_sim = {avg_sim:.4f}")
        return avg_sim
    except Exception as e:
        print(f"Error during similarity calculation via calculator: {e}. Returning 0.0 similarity.")
        return 0.0
...
def main():
    ...
    # 示例参数配置
    params = {
        'dataset_path': "/data/hzy/xh/dtfl/dt-gfn/dt-gfn/gfn/gflownet/priors/all_trees.json",  # 原始树数据集路径
        'output_dir': None,  # 自动生成带时间戳的目录
        'num_sample_data': 7,  # 采样树的数量（此处设为1以加快演示）
        'num_desired_subtrees': 5,  # 每棵采样树生成的子树数量
        
        # 树采样参数 - 更宽松的参数，以便通过质量检查
        'sample_params': {
            'depth_low': 2,      # 从最小深度2开始
            'depth_high': 10,    # 最大深度增至10
            'K_low': 3,          # 最少3个分类
            'K_high': 3,        # 最多5个分类
            'condition_count_low': 3,  # 最少4个条件
            'condition_count_high': 5, # 最多5个条件
        ...
        'generation_params': {
            ...
            'p_deactivate': 0.05,           # 失活概率
            'p_prune': 0.05, # 剪枝概率
            'p_keep_feature': 0.95,     # 保留特征概率
        ...
        # 数据生成参数
        'num_samples_per_class': 15,  # 每个类别生成的样本数
