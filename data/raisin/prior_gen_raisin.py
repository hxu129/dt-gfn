...
csv_path = os.path.join(script_dir, "raisin.csv")
output_dir = script_dir # Save outputs in the same directory

# Global class names for raisin (0: Kecimen, 1: Besni - inferred from data)
GLOBAL_RAISIN_CLASSES_STR = ["0", "1"]

# --- BFS Conversion Function (Adapted for Raisin) ---
def convert_sklearn_tree_to_bfs_numerical_raisin(dt_classifier, feature_names_ordered, global_raisin_classes_str_list):
    """
    Converts a trained scikit-learn DecisionTreeClassifier for Raisin
    into a BFS numerical list format.
    """
    sklearn_tree = dt_classifier.tree_
    num_classes_global = len(global_raisin_classes_str_list)
    ...
    while queue:
        ...
        if total_samples_in_node > 0:
            ...
            for i, class_val_in_tree in enumerate(dt_classifier.classes_):
                try:
                    global_idx = global_raisin_classes_str_list.index(str(int(class_val_in_tree))) # Ensure class is string
                    node_probs_standard_length[global_idx] = float(temp_probs_sklearn_order[i])
                except ValueError:
                    print(f"Warning: Tree class {class_val_in_tree} not found in global list {global_raisin_classes_str_list}.")
        else:
...
# --- Main Script Logic ---
try:
    raisin_full_df = pd.read_csv(csv_path)
except FileNotFoundError:
    print(f"Error: {csv_path} not found.")
    exit()

# Identify feature columns (first 7 columns) and target/split columns
feature_cols = raisin_full_df.columns[:7].tolist()
target_col = 'class'
split_col = 'Split'

# Check if Split column exists
if split_col not in raisin_full_df.columns:
    print(f"Error: '{split_col}' column not found in {csv_path}.")
    # Optional: Add logic here to create a split if needed
    exit()

# Separate fixed test set
fixed_test_df = raisin_full_df[raisin_full_df[split_col] == 'test'].copy()
if fixed_test_df.empty:
     print("Warning: No samples found with Split='test'. Cannot evaluate priors.")
     # Decide how to proceed: maybe create a test set or skip evaluation?
     # For now, we'll continue but evaluation will fail or be meaningless.
     X_fixed_test = pd.DataFrame(columns=feature_cols)
     y_fixed_test = pd.Series(dtype=raisin_full_df[target_col].dtype)
else:
...
initial_train_df = raisin_full_df[raisin_full_df[split_col] == 'train'].copy()
...
# Combine GFN training and fixed test for raisin_0.csv
output_data_for_raisin_0 = pd.concat([gflownet_train_df, fixed_test_df])
output_file_0 = os.path.join(output_dir, "raisin_0.csv")
output_data_for_raisin_0.to_csv(output_file_0, index=False)
...
if len(prior_pool_df) == 0:
    ...
    # Handle this case, maybe exit or use the initial_train_df? For now, exit.
    exit()
...
for i in range(num_prior_trees):
    ...
    possible_classes = sorted(raisin_full_df[target_col].unique())
    y_prior_train_cat = pd.Categorical(y_prior_train, categories=possible_classes)
    ...
    if not X_fixed_test.empty:
        ...
        try:
            report_dict_on_test = classification_report(y_fixed_test, y_pred_on_test, zero_division=0, target_names=GLOBAL_RAISIN_CLASSES_STR, output_dict=True)
            report_str_on_test = classification_report(y_fixed_test, y_pred_on_test, zero_division=0, target_names=GLOBAL_RAISIN_CLASSES_STR)
    ...
    bfs_tree = convert_sklearn_tree_to_bfs_numerical_raisin(clf_prior, feature_cols, GLOBAL_RAISIN_CLASSES_STR)
    all_prior_bfs_trees.append(bfs_tree)

    # Save individual tree
    individual_tree_filename = os.path.join(output_dir, f"raisin_prior_tree_iter_{i+1}.json")
    try:
...
combined_priors_filename = os.path.join(output_dir, "raisin_prior_trees_for_compare.json")
try:
...
evaluation_metrics_filename = os.path.join(output_dir, "raisin_prior_trees_evaluation_metrics.json")
try:
...
print("\nRaisin prior generation and evaluation complete.")
...
try:
    for col in feature_cols:
        min_val = raisin_full_df[col].min()
        max_val = raisin_full_df[col].max()
        bounds.append((float(min_val), float(max_val)))
    print("Bounds calculated.")
    bounds_filename = os.path.join(output_dir, "raisin_feature_bounds.json")
