...
csv_path = os.path.join(script_dir, "breast_cancer.csv")
output_dir = script_dir # Save outputs in the same directory

# Global class names for breast cancer (0: benign, 1: malignant)
GLOBAL_BC_CLASSES_STR = ["0", "1"]

# --- BFS Conversion Function (Adapted for Breast Cancer) ---
def convert_sklearn_tree_to_bfs_numerical_bc(dt_classifier, feature_names_ordered, global_bc_classes_str_list):
    """
    Converts a trained scikit-learn DecisionTreeClassifier for Breast Cancer
    into a BFS numerical list format.

    Args:
        dt_classifier: Trained sklearn.tree.DecisionTreeClassifier instance.
        feature_names_ordered: List of feature names used during training (ordered).
        global_bc_classes_str_list: Global list of class names (e.g., ["0", "1"]).

    Returns:
        list: BFS ordered list of nodes. Node format:
              [is_leaf, feature_idx, threshold, -1, 0, prob_c0, prob_c1]
    """
    sklearn_tree = dt_classifier.tree_
    num_classes_global = len(global_bc_classes_str_list)

    max_depth = sklearn_tree.max_depth
    # Handle cases where tree might be just a root node (depth 0) or empty
    if sklearn_tree.node_count == 0:
        return []
    if max_depth <= 0: # If depth is 0 or less (e.g. -1 from sklearn), size is 1
        bfs_list_size = 1
    else:
    ...
    while queue:
        ...
        if total_samples_in_node > 0:
            ...
            # dt_classifier.classes_ contains the classes seen by *this specific tree* (could be just [0] or [1])
            for i, class_val_in_tree in enumerate(dt_classifier.classes_):
                try:
                    global_idx = global_bc_classes_str_list.index(str(class_val_in_tree))
                    node_probs_standard_length[global_idx] = float(temp_probs_sklearn_order[i])
                except ValueError:
                    print(f"Warning: Tree class {class_val_in_tree} not found in global list {global_bc_classes_str_list}.")
        else:
            # Assign uniform probability if no samples reach the node (e.g., due to max_depth)
        ...
        if sklearn_tree.children_left[current_sklearn_id] == _tree.TREE_LEAF:
            ...
            # Map feature index from sklearn internal potentially reduced set to the original index
            # In this adaptation, we assume feature_names_ordered *is* the full list, so indices match.
...
# --- Main Script Logic ---
try:
    bc_full_df = pd.read_csv(csv_path)
except FileNotFoundError:
    print(f"Error: {csv_path} not found.")
    exit()

# Identify feature columns (first 30 columns) and target/split columns
feature_cols = bc_full_df.columns[:30].tolist()
target_col = 'class'
split_col = 'Split'

# Separate fixed test set
fixed_test_df = bc_full_df[bc_full_df[split_col] == 'test'].copy()
...
initial_train_df = bc_full_df[bc_full_df[split_col] == 'train'].copy()
...
# Combine GFN training and fixed test for breast_cancer_0.csv
output_data_for_bc_0 = pd.concat([gflownet_train_df, fixed_test_df])
output_file_0 = os.path.join(output_dir, "breast_cancer_0.csv")
output_data_for_bc_0.to_csv(output_file_0, index=False)
...
if len(prior_pool_df) == 0:
    print("Error: Prior pool is empty. Cannot generate prior trees.")
    exit()
...
for i in range(num_prior_trees):
    ...
    # Ensure target variable is categorical with all possible classes (0, 1)
    y_prior_train_cat = pd.Categorical(y_prior_train, categories=[0, 1])

    # Train Decision Tree (max_depth=4 for consistency with iris example)
    clf_prior = DecisionTreeClassifier(max_depth=4, random_state=100 + i) # Use different random state per tree
    ...
    try:
        report_dict_on_test = classification_report(y_fixed_test, y_pred_on_test, zero_division=0, target_names=GLOBAL_BC_CLASSES_STR, output_dict=True)
        report_str_on_test = classification_report(y_fixed_test, y_pred_on_test, zero_division=0, target_names=GLOBAL_BC_CLASSES_STR)
    ...
    bfs_tree = convert_sklearn_tree_to_bfs_numerical_bc(clf_prior, feature_cols, GLOBAL_BC_CLASSES_STR)
    all_prior_bfs_trees.append(bfs_tree)

    # Save individual tree
    individual_tree_filename = os.path.join(output_dir, f"breast_cancer_prior_tree_iter_{i+1}.json")
    try:
        with open(individual_tree_filename, 'w') as f:
            json.dump([bfs_tree], f, indent=2) # Store as list containing one tree
...
combined_priors_filename = os.path.join(output_dir, "breast_cancer_prior_trees_for_compare.json")
try:
...
evaluation_metrics_filename = os.path.join(output_dir, "breast_cancer_prior_trees_evaluation_metrics.json")
try:
...
print("\nBreast Cancer prior generation and evaluation complete.")

# Calculate bounds from the full dataset for gflownet_acc.py update
print("\nCalculating feature bounds from full dataset...")
bounds = []
try:
    for col in feature_cols:
        min_val = bc_full_df[col].min()
        max_val = bc_full_df[col].max()
        bounds.append((float(min_val), float(max_val)))
    print("Bounds calculated:")
    # Print bounds for user confirmation/use in next step (optional)
    # for name, bound in zip(feature_cols, bounds):
    #     print(f"  {name}: {bound}")
    # Save bounds to a file for easy copy-paste or programmatic access?
    bounds_filename = os.path.join(output_dir, "breast_cancer_feature_bounds.json")
    ...
    print("Bounds calculation failed. Manual update in gflownet_acc.py might be needed.") 